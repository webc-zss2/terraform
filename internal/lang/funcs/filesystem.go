...
func MakeFileExistsFunc(baseDir string) function.Function {
	return function.New(&function.Spec{
		...
		Impl: func(args []cty.Value, retType cty.Type) (cty.Value, error) {
			...
			if err != nil {
				return cty.UnknownVal(cty.Bool).WithMarks(pathMarks), fmt.Errorf("failed to expand ~: %w", err)
			}
			...
			if err != nil {
				...
				return cty.UnknownVal(cty.Bool).WithMarks(pathMarks), fmt.Errorf("failed to stat %s", redactIfSensitive(path, pathMarks))
			}
			...
			return cty.False.WithMarks(pathMarks), fmt.Errorf("%s is not a regular file, but %q",
